function webRotate360() {
    jQuery(document).ready(() => {
        function getConfigURL() {
            ...
            jQuery('.productView-nav img').each(function replaceImages() {
        ...
        if (configURL) {
            ...
            const containerClass = jQuery('.productView-nav-mobile').is(':visible') ? '.productView-nav-mobile' : '.productView-nav-pc';
            const intervalIdMain = setInterval(() => {
                // Get Slider
                const $mainSlider = jQuery(`${containerClass}.slick-initialized.slick-slider`);
